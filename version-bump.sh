...
BUMP_MODE="none"
if git log -1 | grep -q -i -E "major|breaking"; then
  BUMP_MODE="major"
elif git log -1 | grep -q -i -E "minor|feat"; then
  BUMP_MODE="minor"
elif git log -1 | grep -q -i -E "patch|fix|bug"; then
  BUMP_MODE="patch"
fi
...
then
  ...
  REPO="https://$GITHUB_ACTOR:$TOKEN@$GITHUB_SERVER/$GITHUB_REPOSITORY.git"
